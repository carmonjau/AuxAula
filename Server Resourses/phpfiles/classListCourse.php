<?php
	...
	if (isset($_POST["course"])){
		...
		$result = classListCourse($course);
		echo json_encode($result);
	}

	function classListCourse($course){
		...
		// Making the query
		$sql = "SELECT asignatura, grupo, curso, fecha_inicio, fecha_fin FROM calendario_grupo WHERE curso = ?";
		$aux = mysqli_prepare($connection, $sql);
		mysqli_stmt_bind_param($aux, "s", $course);
		$query = $aux->execute();
