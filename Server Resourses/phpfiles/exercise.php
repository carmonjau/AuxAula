<?php
	...
	if (isset($_POST["subject"]) && isset($_POST["group"]) && isset($_POST["course"])){
		...
		$result = exercises($subject, $group, $course);
		echo json_encode($result);
	}

	function exercises($subject, $group, $course){
		...
		// Making the query
		$sql = "SELECT id, nombre, enlace FROM ejercicio e where e.id IN (SELECT ejercicio FROM relacion_ejercicio_grupo WHERE asignatura = ? AND grupo = ? AND curso = ?)";
