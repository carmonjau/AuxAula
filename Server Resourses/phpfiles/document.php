<?php
	...
	if (isset($_POST["subject"]) && isset($_POST["group"]) && isset($_POST["course"])){
		...
		$result = documents($subject, $group, $course,);
		echo json_encode($result);
	}

	function documents($subject, $group, $course){
		...
		// Making the query
		$sql = "SELECT id, nombre, path, enlace FROM documento d where d.id IN (SELECT documento FROM relacion_documento_grupo WHERE asignatura = ? AND grupo = ? AND curso = ?)";
